...
// are bricks that occupy whole cells, so a duration is literally the
// brick's length in cells.
//
// Interactions (all snap to the grid):
//   • drag a brick        → move it; snaps to the nearest cell
//   • drag the right grip → extend / shorten by whole days
//   • drag a tray brick   → drop it on a slot to schedule (1 day)
//   • click a tray brick  → "arm" it, then click a slot to place it
//   • double-click a brick → send it back to the tray (unschedule)
//
// Storage is the same starts_at / ends_at / lane_y model as the
// Gantt view, so the two stay in sync. Writes go through
// useScheduleSync (local-first outbox + background flush).
export function TimelineGrid({
    ...
    // Lanes: enough to fill the viewport height at 100% zoom. The
    // infinite backdrop fills any remaining space at any zoom / pan.
    ...
    useEffect(() => {
        ...
        const ro = new ResizeObserver((entries) => {
            ...
            if (rect.height > 0) setRows(Math.max(MIN_ROWS, Math.floor(rect.height / CELL)))
            syncGrid()
        })
        ro.observe(el)
    ...
    const [placeAnim, setPlaceAnim] = useState<null | {
        ...
        fill: string; iconName: string | null; title: string; go: boolean
    }>(null)
    ...
    function placeByUser(issueId: string, clientX: number, clientY: number, ghostRect: DOMRect | null = null) {
        ...
        if (!cp) return
        const c = Math.max(0, Math.min(cp.col, cols - 1))
        const r = Math.max(0, Math.min(cp.row, rows - 1))
        ...
        if (vp && issue) {
            ...
            setPlaceAnim({
                ...
                toW: cz, toH: cz, fill, iconName, title: issue.title, go: false,
            })
        }
    ...
    useLayoutEffect(() => {
        ...
        if (focusIssueId) {
            ...
            const c = f ? issueToCell(f, originMs, rows) : null
    ...
    }, [mounted, todayCol, focusIssueId, scheduled, originMs, rows])
    ...
    return (
        ...
        >
            {/* Viewport wash — a static, single-gradient fill (no tiling),
                so it can never drift; the crisp grid is drawn as real
                line elements in the world below. */}
            <div
                ref={bgRef}
                className="pointer-events-none absolute inset-0"
                style={{ backgroundColor: "var(--c-surface-2)", backgroundImage: GRID_BG_IMAGE }}
            />
            ...
            <div ref={worldRef} className="absolute left-0 top-0" style={{ willChange: "transform" }}>
                ...
                {todayCol >= 0 && (
                    <div
                        className="pointer-events-none absolute bg-[color:var(--c-accent)]/[0.06]"
                ...
                {dropPreview && (
                    <div
                        className="pointer-events-none absolute rounded-[6px] border-2 border-[color:var(--c-accent)] bg-[color:var(--c-accent)]/15"
                    ...
                    {weekGroups.map((g) => {
                        ...
                        return (
                            ...
                            >
                                <span
                                    className={cn(
                                        "truncate rounded-md px-1.5 py-0.5 text-[10px] font-bold",
                                        current ? "bg-[color:var(--c-text)] text-white" : "text-[color:var(--c-text-muted)]",
                                    )}
                                >
            ...
            <div
                data-ui
                className="pointer-events-auto absolute bottom-4 right-4 z-30 flex items-center gap-0.5 rounded-full border border-[color:var(--c-border)] bg-[color:var(--c-surface)]/95 p-1 shadow-[var(--shadow-pop)] backdrop-blur"
            >
            ...
            {armed && (
                <div
                    data-ui
                    className="pointer-events-auto absolute left-1/2 top-12 z-30 -translate-x-1/2 rounded-full border border-[color:var(--c-accent)]/30 bg-[color:var(--c-surface)]/95 px-3 py-1.5 text-[11.5px] font-semibold text-[color:var(--c-accent)] shadow-[var(--shadow-pop)] backdrop-blur"
                >
                    Click a slot to place · <button type="button" className="underline" onClick={disarm}>cancel</button>
                </div>
            )}
            ...
            {trayDrag && (() => {
                ...
                return (
                    <div
                        data-drag-ghost
                        className="pointer-events-none fixed z-[60] flex items-center gap-1.5 rounded-[10px] px-2.5 py-1.5 text-[11px] font-bold shadow-[var(--shadow-pop)]"
                        style={{
            ...
            {placeAnim && (
                <div
                    className="pointer-events-none fixed z-[55] flex items-center gap-1.5 overflow-hidden rounded-[8px] px-2 text-[11px] font-bold shadow-[var(--shadow-pop)]"
                    style={{
                        ...
                        background: placeAnim.fill,
                        color: isDarkColor(placeAnim.fill) ? "#ffffff" : "#0a0a0a",
                        transition:
...
const GRID_BG_IMAGE =
    "linear-gradient(135deg, color-mix(in srgb, var(--c-trigger-bg) 18%, transparent), color-mix(in srgb, var(--c-action-bg) 14%, transparent) 35%, color-mix(in srgb, var(--c-output-bg) 14%, transparent) 65%, color-mix(in srgb, var(--c-info-bg) 18%, transparent))"

const GRID_LINE = "rgba(17,24,39,0.09)"
...
function ColHeader({ dateMs, isToday, weekStart, left, top, cell }: { dateMs: number; isToday: boolean; weekStart: boolean; left: number; top: number; cell: number }) {
    ...
    return (
        <div
            className={cn(
                "absolute flex flex-col items-center justify-center gap-0.5 border-r border-[color:var(--c-border)] text-center",
                weekStart && "border-l border-l-[color:var(--c-border-strong)]",
                weekend && "bg-[color:var(--c-surface-2)]",
                isToday && "bg-[color:var(--c-accent)]/10",
            )}
        ...
        >
            <div
                className={cn(
                    ...
                    isToday ? "text-[color:var(--c-accent)]" : "text-[color:var(--c-text-dim)]",
                )}
            ...
            <div
                className={cn(
                    ...
                    isToday ? "text-[color:var(--c-accent)]" : "text-[color:var(--c-text)]",
                )}
...
function Brick({
    ...
    const radius = clamp(cell * 0.2, 5, 12)
...
function FloatingTray({
    ...
    return (
        <div data-ui className="pointer-events-none absolute inset-x-0 bottom-4 z-30 flex justify-center px-4">
            <div className="pointer-events-auto flex max-w-full items-center gap-2 rounded-[16px] border border-[color:var(--c-border)] bg-[color:var(--c-surface)]/95 p-2 shadow-[var(--shadow-pop)] backdrop-blur">
                <div className="shrink-0 pl-1 pr-0.5 text-[9px] font-bold uppercase leading-tight tracking-[0.12em] text-[color:var(--c-text-muted)]">
                    Unsched<br />{items.length}
                </div>
...
function TrayBrick({
    ...
    return (
        <div
            onPointerDown={down}
            className={cn(
                "flex shrink-0 cursor-grab touch-none select-none items-center gap-1.5 rounded-[10px] px-2.5 py-1.5 text-[11px] font-bold active:cursor-grabbing",
                armed && "ring-2 ring-[color:var(--c-accent)] ring-offset-1",
                dragging && "opacity-40",
            )}
            style={{
                background: fill,
                color: fg,
                boxShadow: `inset 0 2px 0 ${shade(fill, 0.22)}, inset 0 -5px 0 ${shade(fill, -0.16)}, 0 1px 2px rgba(15,23,42,0.2)`,
            }}
            title={`${issue.title} • #${issue.issue_number}`}
        >
            <span className="grid h-4 w-4 shrink-0 place-items-center rounded bg-white/25">
            ...
            <span className="max-w-[160px] truncate">{issue.title}</span>
            <span className="shrink-0 font-mono text-[9px] opacity-80">#{issue.issue_number}</span>
        </div>
    )
}
