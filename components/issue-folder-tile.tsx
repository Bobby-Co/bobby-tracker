"use client"

import Link from "next/link"
import { useRouter } from "next/navigation"
import { useState } from "react"
import type { Issue } from "@/lib/supabase/types"
import { IssueTile } from "@/components/issue-tile"
import { StatusChip } from "@/components/status-chip"
import { Modal } from "@/components/modal"

// Tile-view counterpart to the list view's parent-with-children row.
// The parent tile is rendered with a "stack of papers" decoration behind
// it (two offset ledges) so it reads as a folder. A quiet footer shows
// the duplicate count and opens a modal listing each duplicate.
//
// We don't expand inline — CSS grid would shove neighbours around — so
// the modal keeps the grid stable.
export function IssueFolderTile({
    parent,
    duplicates,
    projectId,
    index,
}: {
    parent: Issue
    duplicates: Issue[]
    projectId: string
    index?: number
}) {
    const router = useRouter()
    const [open, setOpen] = useState(false)
    const count = duplicates.length

    return (
        <>
            {/* Wrapper stitches the parent tile + footer into one folder:
                - [&_.card]:rounded-b-none flattens the tile's bottom corners
                  so the footer docks under them.
                - The inner tile renders with interactive={false} so only the
                  wrapper lifts on hover — tile + footer move together. */}
            <div
                className="relative anim-rise transition-transform hover:-translate-y-px [&_.card]:rounded-b-none"
                style={index != null ? ({ ["--i" as string]: index } as React.CSSProperties) : undefined}
            >
                {/* Bottom-only ledges suggesting a stack of papers underneath. */}
                <div
                    aria-hidden
                    className="pointer-events-none absolute left-[10px] right-[10px] bottom-[-6px] h-[12px] rounded-b-[13px] border border-t-0 border-[color:var(--c-border-strong)] bg-white"
                />
                <div
                    aria-hidden
                    className="pointer-events-none absolute left-[5px] right-[5px] bottom-[-3px] h-[8px] rounded-b-[14px] border border-t-0 border-[color:var(--c-border-strong)] bg-white"
                />

                <div className="relative">
                    <IssueTile issue={parent} projectId={projectId} index={0} interactive={false} />
                </div>

                <button
                    type="button"
                    onClick={() => setOpen(true)}
                    aria-label={`Show ${count} duplicate${count === 1 ? "" : "s"}`}
                    className="relative flex w-full items-center justify-between gap-2 rounded-b-[14px] border border-t-0 border-[color:var(--c-border)] bg-[color:var(--c-surface-2)] px-3 py-2 text-[11.5px] font-semibold text-[color:var(--c-text-muted)] transition-colors hover:bg-[color:var(--c-overlay)]"
                >
                    <span className="inline-flex items-center gap-1.5">
                        <FolderIcon />
                        {count} duplicate{count === 1 ? "" : "s"}
                    </span>
                    <span aria-hidden className="text-[10.5px] uppercase tracking-[0.08em] text-[color:var(--c-text-dim)]">
                        View
                    </span>
                </button>
            </div>

            <Modal
                open={open}
                onClose={() => setOpen(false)}
                title={`#${parent.issue_number} · ${count} duplicate${count === 1 ? "" : "s"}`}
                description="These reports were marked as duplicates of this issue. Click one to open its detail page."
                size="md"
            >
                <div className="flex flex-col gap-3">
                    <div className="rounded-[12px] border border-[color:var(--c-border)] bg-[color:var(--c-surface-2)] p-3">
                        <div className="text-[10.5px] font-bold uppercase tracking-[0.08em] text-[color:var(--c-text-muted)]">
                            Original
                        </div>
                        <Link
                            href={`/projects/${projectId}/issues/${parent.id}`}
                            prefetch={false}
                            onClick={() => setOpen(false)}
                            className="mt-1 flex items-center gap-2 text-[13px] font-semibold hover:underline"
                        >
                            <span className="font-mono text-[11px] text-[color:var(--c-text-dim)]">
                                #{parent.issue_number}
                            </span>
                            <span className="min-w-0 flex-1 truncate">{parent.title}</span>
                            <StatusChip status={parent.status} />
                        </Link>
                    </div>

                    <div>
                        <div className="text-[10.5px] font-bold uppercase tracking-[0.08em] text-[color:var(--c-text-muted)]">
                            Duplicates
                        </div>
                        <ul className="mt-1.5 flex flex-col gap-1.5">
                            {duplicates.map((d) => (
                                <li key={d.id}>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            setOpen(false)
                                            router.push(`/projects/${projectId}/issues/${d.id}`)
                                        }}
                                        className="group flex w-full items-center gap-2 rounded-[10px] border border-[color:var(--c-border)] bg-white px-3 py-2 text-left text-[12.5px] transition-colors hover:bg-[color:var(--c-surface-2)]"
                                    >
                                        <span className="font-mono text-[11px] text-[color:var(--c-text-dim)]">
                                            #{d.issue_number}
                                        </span>
                                        <span className="min-w-0 flex-1 truncate text-[color:var(--c-text-muted)] transition-transform group-hover:translate-x-px">
                                            {d.title}
                                        </span>
                                        <StatusChip status={d.status} />
                                    </button>
                                </li>
                            ))}
                        </ul>
                    </div>
                </div>
            </Modal>
        </>
    )
}

function FolderIcon() {
    return (
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            <path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z" />
        </svg>
    )
}
