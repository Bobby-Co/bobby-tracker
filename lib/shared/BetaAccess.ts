...
//
// Deliberately a single owner so the source of truth is easy to swap later (e.g.
// a `tracker.allowlist` table behind an API route) without touching the call
// sites in the callback, the app guard, onboarding, or the page.
//
// CLIENT-SAFE (lives in lib/shared, not lib/server): it reads NEXT_PUBLIC_* so
// the SAME check runs on both the server (OAuth callback) and the client (route
// guards). Keep the allowlist short — public env vars ship in the client bundle.
