...
import { jsonError } from "@/lib/server/http/responses"
...
//
// RateLimiter is stateless; the "warned once" flag lives at module scope so the
// warning stays once-per-isolate no matter how many instances routes construct.

type RateLimitOutcome = { success: boolean }
type CloudflareRateLimiter = { limit: (opts: { key: string }) => Promise<RateLimitOutcome> }
...
function resolveLimiter(binding: RateLimitBinding): CloudflareRateLimiter | null {
    try {
        const { env } = getCloudflareContext()
        const candidate = (env as Record<string, unknown>)[binding]
        if (candidate && typeof (candidate as CloudflareRateLimiter).limit === "function") {
...
/** The unauthenticated-surface rate limiter (Cloudflare bindings today). */
export class RateLimiter {
    ...
    // another's budget.
    clientKey(request: Request, scope: string): string {
        const ip =
    ...
    // enforce returns a 429 Response when the caller is over the limit, or null
    // when the request may proceed. Usage:
    //
    //   const rl = new RateLimiter()
    //   const limited = await rl.enforce("PUBLIC_RL", rl.clientKey(request, "public-submit"))
    //   if (limited) return limited
    async enforce(binding: RateLimitBinding, key: string): Promise<Response | null> {
