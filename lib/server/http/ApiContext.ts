...
// repositories (ctx.access, ctx.issues, …); there is no raw `.from()` here, no
// Supabase construction, and no DB client leaks out of a guard result. Routes get
// `ctx` and reach the store only through its repositories. The pure response
// builders (jsonError/forbidden) stay free functions in ./responses.
...
// ─── result unions — the authz facts + the request unit of work (ctx). ───────
export type AuthSuccess = { ctx: RequestContext; user: User; error: null }
export type AuthFailure = { ctx: RequestContext; user: null; error: Response }
...
export type ProjectSuccess = { ctx: RequestContext; user: User; teamId: string; role: TeamRole; error: null }
export type ProjectFailure = { ctx: RequestContext; user: null; teamId: null; role: null; error: Response }

export type IssueSuccess = { ctx: RequestContext; user: User; projectId: string; teamId: string; role: TeamRole; error: null }
export type IssueFailure = { ctx: RequestContext; user: null; projectId: null; teamId: null; role: null; error: Response }
...
export class ApiContext {
    ...
    async requireUser(): Promise<AuthSuccess | AuthFailure> {
        ...
        if (!user) {
            return { ctx, user: null, error: jsonError("unauthorized", "sign in required", 401) }
        }
        return { ctx, user, error: null }
    }
    ...
    async requireTeam(): Promise<TeamSuccess | TeamFailure> {
        ...
        if (base.error) {
            return { ctx: base.ctx, user: null, team: null, teamId: null, role: null, error: base.error }
        }
        const { ctx, user } = base
        ...
        try {
            ...
            return {
                ctx, user: null, team: null, teamId: null, role: null,
    ...
    async requireProjectAccess(projectId: string): Promise<ProjectSuccess | ProjectFailure> {
        const base = await this.requireUser()
        if (base.error) return { ctx: base.ctx, user: null, teamId: null, role: null, error: base.error }
        const { ctx, user } = base
    ...
    async requireIssueAccess(issueId: string): Promise<IssueSuccess | IssueFailure> {
        ...
        return { ctx, user, projectId, teamId: access.teamId, role: access.role, error: null }
    }
    ...
    private async requireTeamRowAccess(
        ...
        const base = await this.requireUser()
        if (base.error) return { ctx: base.ctx, user: null, teamId: null, role: null, isCreator: false, error: base.error }
        const { ctx, user } = base
        const notFound: TeamRowFailure = { ctx, user: null, teamId: null, role: null, isCreator: false, error: jsonError("not_found", "not found", 404) }
