...
import { VCSAppService } from "./application/vcs-app-service"
import { VCSUserService } from "./application/vcs-user-service"
import { PullRequestService } from "./application/pull-request-service"
import { GithubAppInstance } from "./infrastructure/github-app-instance"
import { GithubUserInstance } from "./infrastructure/github-user-instance"
import { githubWebhookVerifier } from "./infrastructure/github-webhook-verifier"
import { createServicePullRequestStore } from "./infrastructure/supabase-pull-request-store"
...
export function resolveVcsAppInstance(project: VcsProviderBinding): VCSAppInstance | null {
    ...
    if (!or) return null
    return new GithubAppInstance(project.github_installation_id, or[0], or[1])
}
...
export function resolveVcsUserInstance(repo: VcsRepoCoords, token: string): VCSUserInstance | null {
    ...
    if (!or) return null
    return new GithubUserInstance(token, or[0], or[1])
}
